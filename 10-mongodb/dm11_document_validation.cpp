        ...
        collection zombies = db.create_collection(
                ...
                make_document(
                        kvp("validator", make_document(
                                kvp("brains", make_document(kvp("$gt", 0)))))));

        try {
            // Insert a document passing validation
            auto res = zombies.insert_one(
                    make_document(kvp("name", "Bloody Betty"), kvp("brains", 3)));

            std::cout << "Bloody Betty passed document validation!" << std::endl;

            // Insert a document failing validation, which should throw.
            auto res2 = zombies.insert_one(
                    make_document(kvp("name", "Undead Fred"), kvp("brains", 0)));
